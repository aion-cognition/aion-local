...
/**
 * How often recall answered on a degraded cue stage. The exercise measured 2.2% all-time and
 * 17% under worker load, with the degraded pack indistinguishable from a healthy one in item
 * count and token size — so nothing operational could tell a bad hour from a good one. The
