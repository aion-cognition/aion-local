 ...
 * its ledger key applied. A rolling window of raw samples, not a running average: a measured
 * p95 of this figure showed 95% queueing, and a mean would hide exactly the tail a freshness
 * signal exists to catch. `reflection-queue.ts` deletes a job's row on completion
