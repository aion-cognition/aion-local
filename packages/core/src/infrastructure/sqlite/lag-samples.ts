 ...
 * its ledger key applied. A rolling window of raw samples, not a running average: the p95 of
 * this figure measured 95% queueing, and a mean would hide exactly the tail a freshness signal
 * exists to catch. `claim.ts` deletes a job's row on completion, so nothing durable survives
 * the run itself; this is the only record of it.
 */
