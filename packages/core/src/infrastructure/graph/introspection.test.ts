...
function indexes(dimension: number): VectorIndexInfo[] {
  return VECTOR_INDEX_NAMES.map((name) => ({
    name,
...
describe('assertVectorIndexDimensions', () => {
  it('accepts indexes built at the embedding model dimension', () => {
    expect(() => {
      assertVectorIndexDimensions(indexes(768), 768, 'nomic-embed-text');
    }).not.toThrow();
  });
  ...
  it('reports a missing index separately from a mis-dimensioned one', () => {
    ...
    expect(() => {
      assertVectorIndexDimensions(partial, 768, 'nomic-embed-text');
    }).toThrow(VectorIndexMissingError);
  });
  ...
  it('does not judge an index whose dimension the server did not report', () => {
    ...
    expect(() => {
      assertVectorIndexDimensions(unreported, 768, 'nomic-embed-text');
    }).not.toThrow();
  });
});
