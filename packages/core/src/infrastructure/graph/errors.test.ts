...
describe('isGraphUnavailable', () => {
  it('is true for the driver codes that mean the server was never reached', () => {
    expect(isGraphUnavailable(driverError('connect ECONNREFUSED', 'ServiceUnavailable'))).toBe(
      true,
    );
    expect(
      isGraphUnavailable(driverError('server at address is no longer available', 'SessionExpired')),
    ).toBe(true);
  });
  ...
  it('is true for the connection pool timing out', () => {
    const timedOut = driverError(
      'Connection acquisition timed out in 10000 ms. Pool status: Active conn count = 0',
      'N/A',
    );
  ...
  it('is false for an answer the server gave, however unwelcome', () => {
    expect(
      isGraphUnavailable(
        driverError('constraint violated', 'Neo.ClientError.Schema.ConstraintValidationFailed'),
      ),
    ).toBe(false);
