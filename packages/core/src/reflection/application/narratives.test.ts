...
const OUTPUT = {
  summary: 'the session shipped the reflection worker',
  narrative: 'The pair shipped the reflection worker, then gated its re-run on the ledger key.',
};
...
describe('session close', () => {
  ...
  it('compresses the session into a narrative with its provenance and vector', async () => {
    ...
    expect(node.properties[MEMORY_PROPERTIES.summary]).toBe(OUTPUT.summary);
    expect(node.properties[MEMORY_PROPERTIES.text]).toBe(OUTPUT.narrative);
  ...
  it('embeds the narrative body, which is the property a later backfill would embed', async () => {
    ...
    expect(embed).toHaveBeenCalledWith([OUTPUT.narrative]);
  });
