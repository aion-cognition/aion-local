...
const SYSTEM_PROMPT = [
  ...
  'PRECEDES when one item happened before another in a way that matters, RELATED_TO when two',
  'items are meaningfully connected but no directed type fits, and ANALOGOUS_TO when two',
  'items are similar in kind without one causing, enabling, or preceding the other.',
