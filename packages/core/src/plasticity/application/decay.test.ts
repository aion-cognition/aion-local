...
import { decaySweepCounters, recordDecaySweep } from '../../infrastructure/sqlite/decay-counters.js';
...
describe('decay sweep counters', () => {
  ...
  it('accumulates across runs and keeps the latest run time', () => {
    recordDecaySweep(store.db, { edgesScanned: 10, edgesDecayed: 6, at: '2026-01-01T00:00:00.000Z' });
    recordDecaySweep(store.db, { edgesScanned: 4, edgesDecayed: 1, at: '2026-01-02T00:00:00.000Z' });
  ...
  it('advances lastRunAt on an empty sweep, the signal that separates quiet from stalled', () => {
    recordDecaySweep(store.db, { edgesScanned: 0, edgesDecayed: 0, at: '2026-01-03T00:00:00.000Z' });
