...
**What happens.** `extractCues` (`packages/core/src/recall/application/cues.ts:305`)
...
`model_error` (`cues.ts:187-212`). A response that parses but fails
`CueModelOutputSchema.safeParse` is `invalid_output` (`cues.ts:321-325`). All three call
`degradedResult`, which builds cues from the caller's own query and summary text verbatim
(query at weight 3, summary at weight 1) and never derives terms from it (`cues.ts:242-256`).
Only a successful extraction is cached (`cues.ts:334`), so a degraded call never poisons a
later one for the same input.
...
Verified by the unit suite, not live-induced: `cues.test.ts:154` (a degraded result is
never cached), `:225` and `:234` (both `invalid_output` shapes), `:247` (timeout). 37/37
pass (`npx vitest run --project unit packages/core/src/recall/application/cues.test.ts`).
...
(`packages/core/src/recall/application/stage-reads.ts:50-69`) then tries to embed the
...
one list (`recall.ts:279-291`). With no vectors, BM25, entity resolution, recency, and
...
(`packages/core/src/recall/application/seeds.ts:186-203`). `selectSeeds` also counts
...
unavailable` (`seeds.ts:411-414`). The recency leg always issues exactly one query, which
...
(`recall.ts:294-296`) and the pack assembles empty.
...
process life (`packages/core/src/infrastructure/graph/connection.ts:214-252`); its pool
...
**What happens.** `resonate` (`packages/core/src/recall/application/resonance.ts:161-187`)
...
**Diagnose.** `aion doctor`'s `neo4j-bolt` check. Live, `tools.ts:129`'s `tool call failed`
...
with no queue row and no enqueue callback.
...
acquire a pooled connection, 30s of transaction retries. Both meet or exceed the
...
(`packages/core/src/infrastructure/graph/connection.ts:205-207`, applied at `:224-226`). A
...
`runService`'s catch writes it to stderr and returns exit code 1 before `listen()` is ever
called (`packages/mcp/src/run.ts:37-40`). No listener binds; no partial service exists.
...
the session map (`packages/mcp/src/session-registry.ts`) rather than closing right away. Two
backstops bound it instead of a leak: the registry evicts down to `MAX_SESSIONS = 512` on
every new session past the cap (logged as `mcp session evicted`), and `SessionIdleSweeper`
...
| `AION_EMBED_KEEP_ALIVE` (`ollama.embedKeepAlive`) | -1 | Raised to a positive number of seconds, Ollama gives the embed model's 1.2GB of unified memory back after that idle window instead of holding it resident forever. | Every reload after the window lands inside recall's embed stage, which the harness hooks await synchronously: measured against warm embeds under a second, reloads added 1 to 16 seconds, and the worst carried a session-start hook past its 10s timeout. Boot softens the first hit either way: `warmEmbedModel` (`packages/mcp/src/bootstrap.ts`) embeds one probe at service start, fail-open, so a cold Ollama pays its load there rather than on the first recall. | New; no spec pin. |
...
| `AION_KEYED_NARRATIVE_EPISODES` / `AION_KEYED_NARRATIVE_SENTENCES` / `AION_KEYED_NARRATIVE_EPISODE_CHARS` | 120 / 12 / 4000 | How much of a session one synthesis reads and how long its answer runs, on the keyed route only. Three times the episodes and twice the sentences of the local numbers (`AION_REFLECTION_MAX_NARRATIVE_EPISODES` 40, six sentences, `AION_REFLECTION_MAX_NARRATIVE_EPISODE_CHARS` 2000), which is what a 120-episode session takes in one Haiku call. The same three govern the day and week rollups and the subject consolidation, so one rule sizes every synthesis source. | A longer prompt and a longer answer per call. A session past 120 episodes is read in chunks rather than clipped, so this number sets the chunk size and one narration costs `ceil(n / 120) + 1` calls. | New with the two profiles. Inert without a key: `narrativeScale` reads one set of numbers or the other off the resolved reflect route, so a local install never sees these whatever they are set to. |
| `AION_PROPOSAL_RESOLUTION` / `AION_RESOLUTION_BATCH` (`maintenance.proposalResolution`, `.resolutionBatch`) | true / 10 | Open proposals decided on their merits in the run that reads them, oldest first across both queues, instead of waiting for a person and then for the hygiene horizon. Each row costs two model calls and a handful of graph reads, so ten is an hour's modest spend. | A larger batch puts more of one window into model calls, and a wrong verdict reaches the graph an hour after the row was filed rather than a fortnight. Off, both queues wait for `aion proposals`. | New with the autonomous queues. Nothing about it is one-way: every verdict is ledgered under `introspection:resolution:` with both passes' reasons, and `aion unsupersede`, `aion unmerge`, and `aion proposals reopen` reverse a resolver decision the same way they reverse a person's. |
...
failed, how many no metric scored, what a run typically costs, and the last window's outcome.
An operation that has never been selected says so rather than reporting zeroes.
